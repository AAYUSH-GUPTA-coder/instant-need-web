...
function InvoicePrint({ order }: { order: OrderDTO }) {
  ...
  return (
    <div className="hidden print:block" style={{ fontFamily: "Arial, sans-serif", fontSize: "10px", color: "#111" }}>
      <div style={{ width: "100%", boxSizing: "border-box", border: `${BORDER_WIDTH} solid ${BORDER}`, borderRight: `2px solid ${BORDER}`, padding: 16 }}>
      ...
      <table style={{ ...table, marginTop: 10, borderCollapse: "collapse" }}><tbody><tr><td style={{ ...cellStyle, width: "50%", verticalAlign: "top" }}><b>Billed To:</b><div>{customerName}</div><div>{addr.addressLine1}{addr.addressLine2 ? `, ${addr.addressLine2}` : ""}<br />{addr.city}, {addr.state} {addr.postalCode}</div><div>Mobile No : {addr.phoneNumber || "—"}</div><b>GSTIN/UIN : {customerGstin}</b></td><td style={{ ...cellStyle, width: "50%", verticalAlign: "top" }}><b>Shipped To:</b><div>{customerName}</div><div>{addr.addressLine1}{addr.addressLine2 ? `, ${addr.addressLine2}` : ""}<br />{addr.city}, {addr.state} {addr.postalCode}</div><div>Mobile No : {addr.phoneNumber || "—"}</div><b>GSTIN/UIN : {customerGstin}</b></td></tr></tbody></table>
...
export default function OrderConfirmationPage({ params }: ConfirmationPageProps) {
  ...
  const invoiceDisplay = order.invoiceNumber || order.orderNumber || orderRef(order.id);
  const addr = order.shippingAddress;
  return <>
    <style>{`@media print { @page { margin: 8mm 12mm; size: A4 portrait; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } [data-sonner-toaster], [data-sonner-toast] { display: none !important; } }`}</style>
